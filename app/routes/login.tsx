...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  // If the user is already authenticated redirect to /todos
...
export default function Login() {
  const { googleEnabled, githubEnabled } = useLoaderData<typeof loader>();
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          Welcome to the Remix Full Stack Template
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            {githubEnabled && (
              <Form action="/auth/github" method="post" className="w-full">
                <button
                  type="submit"
                  className="w-full flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors"
                >
                  <svg
                    className="w-5 h-5 mr-2"
                    fill="currentColor"
            ...
            {!googleEnabled && !githubEnabled && (
              <div className="text-center text-gray-500">
                ...
                <p className="text-sm mt-2">
                  Please set up your environment variables for Google or GitHub
                  OAuth.
                </p>
              </div>
            )}
